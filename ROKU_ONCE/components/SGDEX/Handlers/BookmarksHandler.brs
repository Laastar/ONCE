...
Sub SaveBookmark(TimeStamp as Integer, Program as String)
    sec = createObject("roRegistrySection", "MySection")
    sec.Write(Program, TimeStamp)
    sec.Flush()
...
Function GetBookmark(Program as String) as Integer
    sec = createObject("roRegistrySection", "MySection")
    if sec.Exists(Program)
        BookmarkTime =  sec.Read(Program)
    end if
    content = m.top.content
